...
Saccades %>% 
  ...
  ggplot() +
  aes(Amplitude, color=SearchOrCount, fill=SearchOrCount) +
  geom_density(alpha=0.2) +
  theme_minimal() +
  labs(title="Amplitude of saccades", subtitle="In a count and search task.")
...
```{r}
mGaus <- lm(Amplitude ~ SearchOrCount, data=Saccades)
...
Saccades <- Saccades %>% filter(Amplitude > 0) %>% 
  mutate(LogAmplitude = log(Amplitude))

mLog <- lm(LogAmplitude ~ SearchOrCount, data=Saccades)
...
summary(abs(pm2-Saccades$LogAmplitude))
```
...
```{r}
par(mfrow=c(1,2))
residuals(dGaus) %>% plot(main="Gaussian")
residuals(dLog) %>% plot(main="Log")
...
Saccades <- Saccades %>% 
  mutate(
    LongSaccade = if_else(Duration > mean(Saccades$Duration) + 2 * sd(Saccades$Duration), 1, 0) 
  )
```
...
mLogit <- glm(LongSaccade ~ SearchOrCount, data = Saccades, family = "binomial")
...
**It's significant results but our model is so fucking shit that I would never ever put it into words.**
...
```{r}
Fixations %>% 
  drop_na(SearchOrCount) %>% 
  ggplot(aes(x = Duration, fill = SearchOrCount, color = SearchOrCount)) +
